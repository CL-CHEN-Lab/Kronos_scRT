...
data <-
    foreach(
       ...
       inner_join(read_csv(opt$file[i],col_types = cols()) %>%
...
data = data %>%
        mutate(
            ...
            is_high_dimapd = ifelse(is.na(threshold_Sphase),is_high_dimapd,
                                    ifelse(normalized_dimapd > threshold_Sphase, T, F)),
            is_noisy = ifelse(is_high_dimapd, T, is_noisy)
        )

median_ploidy_G1_G2_cells = data %>%
        filter(ifelse(is.na(threshold_G1G2phase),is_noisy == F,is_noisy == F &
...
data = data %>%
        ...
        mutate(Type = ifelse(
            ...
            'S-phase',
            ifelse( 
                ifelse(is.na(threshold_G1G2phase),
...
p = data %>%
    ...
    geom_vline(aes(xintercept = median_ploidy_G1_G2_cells)) + facet_wrap(group~basename,scales = 'free') +
