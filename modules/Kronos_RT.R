...
suppressMessages(ggsave(
    p,
    filename = paste0(opt$out, '/', opt$output_file_base_name, '_plot.png')
))
...
suppressMessages(ggsave(
    ...
    filename = paste0(
        ...
        '/',
        opt$output_file_base_name,
        '_percentage_of_replicating_cells.png'
    )
))
...
png(
    paste0(
        ...
        '/',
        opt$output_file_base_name,
        '_correlation_plot_per_cell_before_filter.png'
    )
)
...
png(
    paste0(
        ...
        '/',
        opt$output_file_base_name,
        '_correlation_plot_per_cell_after_filter.png'
    )
)
...
suppressMessages(ggsave(
    ...
    filename = paste0(
        opt$out,
        opt$output_file_base_name,
        '_percentage_of_replicating_cells_after_filtering.png'
    )
))
...
if (opt$plot) {
    ...
    if (!'region' %in% names(opt)) {
        for (i in 1:length(Chr_Size$chr)) {
            chrsize = as.numeric(Chr_Size$size[i])
            region = round(runif(1, min = chrsize * .2, max = chrsize - (chrsize * .25) ), 0)
            Chr = Chr_Size$chr[i]
            Start = region
            End = round((region + chrsize * .2), 0)
...
png(
    paste0(
        ...
        opt$output_file_base_name
        ,
        '_RT_distribution_plot_calculated_RT_vs_reference.png'
    )
)
...
#UPDATE: removed T25_75 function from %dopar% (no need to redefine the function each time)
...
if (opt$Var_against_reference) {
    ...
    suppressMessages(ggsave(
        ...
        filename = paste0(
            ...
            '/',
            opt$output_file_base_name,
            '_variability_plot_Early_Late_ref_RT.png'
        )
    ))
