...
distance = foreach(names1 = 1:(length(lines) - 1), .combine = cbind) %:%
    foreach(names2 = (names1 + 1):length(lines),
            .combine = cbind) %do% {
...
clusters <-
    hclust(dist(data_cluster[!names(data_cluster) %in% c('chr', 'start', 'end')]))
...
suppressMessages(
    ggsave(
        ...
        limitsize = FALSE,
        device = cairo_pdf,
        width = unit(3 * length(lines), 'cm'),
        height = unit(nK, 'cm')
    )
)

data_cluster %>% write_tsv(file.path(
    opt$out,
    paste0(
        'Changing_bins_with_th_',
        opt$deltaRT_threshold,
