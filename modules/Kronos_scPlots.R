...
option_list = list(
    make_option(
        ...
        type = "character",
        default = NULL,
        help = "A Tab separated file containing in each column scRT_Tracks and scCNV files paths. Alternative to -R and -C options.",
        metavar = "character"
    ),
    make_option(
        c("-R", "--scRT_Tracks"),
    ...
    make_option(
        ...
        type = "character",
        default = "B",
        help = "What type of data to plot for the single cell tracks: ('B'=Binarized, 'CNV'=Copy number variation, 'log2'=log2(CNV_Cell/CNV_mean_G1/G2_cells) or 'all'= one file per option) [default= %default]",
        metavar = "character"
    ),
    make_option(
        c("-r", "--region"),
...
#create directory
if (!dir.exists(file.path(opt$out, 'regions'))) {
...
if ('extra_RT_track' %in% names(opt)) {
    opt$extra_RT_track = read_tsv(opt$extra_RT_track, col_types = cols(chr =
                                                                           'c')) %>%
        ...
        if (opt$CNV_values == 'B') {
            ...
            p1 = plot + geom_line(
                ...
                scale_fill_gradient(
                    ...
                    limits = c(-1.5, 1.5),
                    breaks = c(-1.5, 0, 1.5)
                ) + labs(fill = expression(over(S[CNV[i]],
                                                bar(G1 / 2[CNV]))))
            ggsave(
                plot = p1,
                filename = paste0(
