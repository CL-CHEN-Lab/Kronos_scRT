...
option_list = list(
    ...
    make_option(
        ...
        type = "numeric",
        default = 1,
        help = "Annotatation to use to calculate pvalues [default= %default]",
        metavar = "numeric"
    ),
    make_option(
        c("-B", "--between_groups"),
    ...
    make_option(
        ...
        default = 10000,
        help = "number of iterations to caklculate boostrap [default= %default]",
        metavar = "numeric",
        type = "numeric"
    ),
    make_option(
...
if ('Cat2' %in% names(data) & !opt$pval) {
    ...
    if(opt$pval & ! opt$between_groups){
        ...
        pval = foreach(
            ...
            pval_TW = foreach (
                ...
                Boot_Strapped = foreach(
                    ...
                    Group1_n = sum(data_g$Cat1 == groups$Group1[i])
                    #bins belonging to the second group
                    Group2_n = sum(data_g$Cat1 == groups$Group2[i])
        ...
        pval = foreach(
            ...
            pval_TW = foreach (
                ...
                Bins_for_shuffling=data_g %>% group_by(group,chr,start,end)%>%summarise()%>%ungroup()
                ...
                Boot_Strapped = foreach(
                    ...
                    Group2 = Bins_for_shuffling[sample(
                        ...
                        inner_join(data_g, by = c("chr", "start", "end"))%>%
    ...
    t=t%>%mutate(Cat1=factor(Cat1,levels = unique(Cat1)),
    ...
    if(opt$pval & ! opt$between_groups){
        ...
        pval=pval%>%
            mutate(a=as.numeric(factor(Cat1,levels(t$Cat1))),
                   ...
                   Statistically=case_when(
                       adj_pval==0 & iterations > 2000 ~ 'significant',
                       adj_pval < 0.05 & adj_pval > 0 ~ 'significant',
                       T ~ 'non-significant'),
                   adj_pval = ifelse(
                       adj_pval == 0,
                       paste0(
